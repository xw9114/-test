...
- 默认开启智能复核：低置信度、无完整选项文本、图片题、填空/简答等高风险题会再次检查；高置信度且选项内容匹配明确的题会跳过复核以节省 token。
- 选择/判断题会优先按模型返回的“选项完整内容”反向匹配页面选项，降低 A/B/C 字母错位导致的误填。
- 同一题干、题型和选项会缓存答案，重复出现时直接复用，减少重复请求。
...
| 智能复核答案 | 开启 | 只复核高风险答案；比每题复核更省 token，同时保留纠错能力 |
...
- 保持“智能复核答案”开启；明确、简单、已返回完整选项内容的题会自动跳过复核。
- 首次在新课程/新题型中使用时，关闭“自动翻页并提交”，先人工抽查回填结果。
- 如果日志中经常出现低置信度，通常说明题干/选项提取不完整、图片无法识别，或模型本身能力不足；请把运行日志中的题干样本发出来继续适配。

参考 `fysh1010/chaoxing-script` 后，当前脚本也会要求模型返回 `answerTexts`（正确选项完整内容），再用精确匹配、包含关系和编辑距离把内容匹配回页面选项。这样当模型返回的字母与选项内容冲突时，会优先相信内容匹配结果。

省 token 策略：

- 题干和选项进入模型前会做长度上限裁剪，避免整页噪声进入 prompt。
